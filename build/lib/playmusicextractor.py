...
# playmusicextractor - decrypt MP3 files from Google Play Music offline storage (All Access)
#
...
class PlayMusicExtractor:
    ...
    def get_info(self):
        ...
        cursor.execute("""SELECT Title, Album, AlbumArtLocation, Artist, AlbumArtist, Composer, Genre, Year, Duration,
    ...
    def get_outfile(self):
        ...
        destination_dir = self.normalize_filename(self.info["Album"])
	...
	response = urllib2.urlopen(self.info["AlbumArtLocation"])
	print(self.info["AlbumArtLocation"])	
	imagedata=response.read()
		

	file = eyed3.load(outfile)
	file.initTag()
	file.tag.artist = self.info["Artist"]
	file.tag.album = self.info["Album"]
	file.tag.title = self.info["Title"]
	file.tag.performer = self.info["AlbumArtist"]
	file.tag.composer = self.info["Composer"]
	file.tag.genre = self.info["Genre"]
	file.tag.date = str(self.info["Year"])
	file.tag.length = str(self.info["Duration"])
	file.tag.discnumber = str(self.info["DiscNumber"])
	file.tag.compilation = str(self.info["Compilation"])
	file.tag.albumartist = self.info["AlbumArtist"]
	file.tag.tracknumber = str(self.info["TrackNumber"])
	file.tag.images.set(3, imagedata , "image/jpeg" ,u"FRONT_COVER")
	file.tag.save()
